...
- Puzzles from **sudoku-gen** (client-side, no backend). Note: it's not a true generator — it ships 40 seed puzzles (10 per difficulty) and disguises them with rotations/band-shuffles/digit relabeling. "Evil" maps to its `expert`. A possible future feature is a real seeded generator so numbered puzzles become reproducible ("Select a puzzle 12,345" like the original); the puzzle number currently displayed is decorative randomness.
...
- **Side keypad** (1–9, ⌫, ✎ pencil toggle) hidden on desktop by default, shown on touch — mirrors the original; overridable in Options.
